...
public class OAuthCallbackListener extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException {
    ...
    if (authorizationCode != null && authorizationCode.length() > 0) {
      ...
      final String REDIRECT_URI = "http://wmiig.com:8080/callback";
      final String CLIENT_ID = "lolololo";
      final String CLIENT_SECRET = "lolololo";

      // Generate POST request
      HttpPost httpPost = new HttpPost(TOKEN_ENDPOINT + "?grant_type=" + URLEncoder.encode(GRANT_TYPE, StandardCharsets.UTF_8.name()) + "&code=" + URLEncoder.encode(authorizationCode, StandardCharsets.UTF_8.name()) +
        "&redirect_uri=" + URLEncoder.encode(REDIRECT_URI, StandardCharsets.UTF_8.name()) +"&client_id=" + URLEncoder.encode(CLIENT_ID, StandardCharsets.UTF_8.name()));
      ...
      String requestUrl = "https://graph.facebook.com/v2.8/me/friends";
